...
public:
  ...
  static void crossProduct(double (&out)[3], const double (&a)[3],
                           const double (&b)[3]);
  ...
  static double normalizeRads(double radians) {
    return radians - floor(radians / (2 * M_PI)) * 2 * M_PI;
  }
  ...
  static void multiply(double (&out)[3], const double (&m)[3][3],
                       const double (&v)[3]);
...
public:
  ...
  void getT(float &m11, float &m12, float &m13, float &m21, float &m22,
            float &m23, float &m31, float &m32, float &m33);

  // set the transformation from EEPROM
  void setT(float m11, float m12, float m13, float m21, float m22, float m23,
            float m31, float m32, float m33);

  void setTinvFromT();

  // Calculate third reference star from two provided ones. Returns false if
  // more or less than two provided
  bool calculateThirdReference();

  HorizCoord toInstrumentCoord(EqCoord eq);
  EqCoord toReferenceCoord(HorizCoord h);

  double polErrorDeg(double lat, Err sel);
  unsigned char refs = 0; // number of reference stars

  void addReferenceCoord(HorizCoord h, EqCoord e);
...
protected:
  ...
  void toInstrumentDeg(double &axis1, double &axis2, double angle1,
                       double angle2) const;

  // Convert axis axis1/axis2 coordinates to  reference angle1/angle2
  // coordinates (all values in degrees)
  void toReferenceDeg(double &angle1, double &angle2, double axis1,
                      double axis2) const;
  EqCoord toReferenceCoord(TakiHorizCoord h);
  void addReference(TakiHorizCoord h, EqCoord e);
  ...
  // Convert reference angle1/angle2 coordinates to instrument coordinates (all
  // values in radians)
  void toInstrument(double &axis1, double &alt, double angle1,
                    double angle2) const;

  // Convert axis axis1/axis2 coordinates to  equatorial hour angle/angle2
  // coordinates (all values in radians)
  void toReference(double &angle1, double &angle2, double axis1,
                   double axis2) const;

  double T[3][3]; // Transformation matrix from equatorial angle1/angle2 cosines
                  // to axis axis2/axis1 cosines
  double Tinv[3][3]; // Inverse of the above

  double dcAARef[3][3]; // axis1/axis2 direction cosine vectors for the three
                        // reference stars, indexed by reference first
  double dcHDRef[3][3]; // angle1/angle2l direction cosine vectors for the three
                        // reference stars, indexed by reference first
